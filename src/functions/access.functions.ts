...
export const redeemAccessCode = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    try {
      ...
      if (acErr) return { ok: false as const, error: "Gagal memvalidasi kode." };
      if (!ac || !ac.is_active) {
      ...
      if (!existing) {
        ...
        if (insErr) return { ok: false as const, error: "Gagal mendaftarkan perangkat." };
      } else {
        await supabaseAdmin
          .from("code_devices")
      ...
      if (sessErr) return { ok: false as const, error: "Gagal membuat sesi." };
      ...
      return {
        ok: false as const,
        error: "Konfigurasi server belum lengkap. Hubungi admin aplikasi.",
      };
    }
  });
...
export const verifySession = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const ac = sess.access_codes as unknown as {
      code: string;
      label: string | null;
      is_active: boolean;
    } | null;
